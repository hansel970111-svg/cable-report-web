...
describe('ReportEditor save feedback', () => {
  it('announces a successful save through the polite live region', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('status')).toHaveTextContent('已保存 report.pdf。');
    });
...
describe('browserReportServices', () => {
  it('imports raw rows once and posts the canonical draft to generate-report', async () => {
    ...
    expect(JSON.parse(String(generateInit.body))).toMatchObject({
      ...
      cableType: 'Cat 5e',
      site: 'M138-DE46',
      records: [{ cableLabel: '#1', dateTime: '10-07-2026 09:00:00 AM' }],
    });
